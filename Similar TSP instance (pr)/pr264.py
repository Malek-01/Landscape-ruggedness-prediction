...
problem = tsplib95.load('C:/Users/\Malek/Desktop/Evostar 2023/Experiment/Third experiments/jorlib-master/jorlib-master/jorlib-core/src/test/resources/tspLib/tsp/pr264.tsp')
graph = problem.get_graph()
distance_matrix = networkx.to_numpy_matrix(graph)
distance_matrix = np.squeeze(np.asarray(distance_matrix))

for k in range(30):
    dist = np.zeros((264,264))
    for i in range(263):
        for j in range(i+1, 264):
            dist[i][j] = distance_matrix[i][j] 
            dist_list = [(0,0,0) for x in range(34716)]  
    l = 0
    #for l in range(1000):
    while l < 34716:   
        for i in range(263):
    ...
    problem_fit = mlrose.TSPOpt(length = 264, fitness_fn = fitness_dists, maximize=False)
...
df.to_excel("TSP_pr_264.xlsx", index=True, startrow=0)