...
problem = tsplib95.load('C:/Users/\Malek/Desktop/Evostar 2023/Experiment/Third experiments/jorlib-master/jorlib-master/jorlib-core/src/test/resources/tspLib/tsp/pr107.tsp')
graph = problem.get_graph()
distance_matrix = networkx.to_numpy_matrix(graph)
distance_matrix = np.squeeze(np.asarray(distance_matrix))

for k in range(30):
    dist = np.zeros((107,107))
    for i in range(106):
        for j in range(i+1, 107):
            dist[i][j] = distance_matrix[i][j] 
            dist_list = [(0,0,0) for x in range(5671)]  
    l = 0
    #for l in range(1000):
    while l < 5671:   
        for i in range(106):
    ...
    problem_fit = mlrose.TSPOpt(length = 107, fitness_fn = fitness_dists, maximize=False)
...
df.to_excel("TSP_pr_107.xlsx", index=True, startrow=0)