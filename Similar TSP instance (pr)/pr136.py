...
problem = tsplib95.load('C:/Users/\Malek/Desktop/Evostar 2023/Experiment/Third experiments/jorlib-master/jorlib-master/jorlib-core/src/test/resources/tspLib/tsp/pr136.tsp')
graph = problem.get_graph()
distance_matrix = networkx.to_numpy_matrix(graph)
distance_matrix = np.squeeze(np.asarray(distance_matrix))

for k in range(30):
    dist = np.zeros((136,136))
    for i in range(135):
        for j in range(i+1, 136):
            dist[i][j] = distance_matrix[i][j] 
            dist_list = [(0,0,0) for x in range(9180)]  
    l = 0
    #for l in range(1000):
    while l < 9180:   
        for i in range(135):
...
df.to_excel("TSP_pr_136.xlsx", index=True, startrow=0)