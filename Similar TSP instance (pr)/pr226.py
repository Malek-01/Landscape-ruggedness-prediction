...
problem = tsplib95.load('C:/Users/\Malek/Desktop/Evostar 2023/Experiment/Third experiments/jorlib-master/jorlib-master/jorlib-core/src/test/resources/tspLib/tsp/pr226.tsp')
graph = problem.get_graph()
distance_matrix = networkx.to_numpy_matrix(graph)
distance_matrix = np.squeeze(np.asarray(distance_matrix))

for k in range(30):
    dist = np.zeros((226,226))
    for i in range(225):
        for j in range(i+1, 226):
            dist[i][j] = distance_matrix[i][j] 
            dist_list = [(0,0,0) for x in range(25425)]  
    l = 0
    #for l in range(1000):
    while l < 25425:   
        for i in range(225):
    ...
    problem_fit = mlrose.TSPOpt(length = 226, fitness_fn = fitness_dists, maximize=False)
...
df.to_excel("TSP_pr_226.xlsx", index=True, startrow=0)