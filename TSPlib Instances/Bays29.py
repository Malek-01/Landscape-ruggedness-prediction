...
problem = tsplib95.load('testdata/bays29.tsp.txt')
...
for k in range(30):
    ...
    for i in range(28):
        for j in range(i+1, 29):
            ...
            dist_list = [(0,0,0) for x in range(406)]  
    l = 0
    #for l in range(1000):
    while l < 406:   
        for i in range(28):
    ...
    problem_fit = mlrose.TSPOpt(length = 29, fitness_fn = fitness_dists, maximize=False)
...
df.to_excel("TSP_Bays_29.xlsx", index=True, startrow=0)
