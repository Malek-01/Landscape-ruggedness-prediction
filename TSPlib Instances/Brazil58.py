...
problem = tsplib95.load('testdata/brazil58.tsp')
...
for k in range(30):
    ...
    for i in range(57):
        for j in range(i+1, 58):
            ...
            dist_list = [(0,0,0) for x in range(1653)]  
    l = 0
    while l < 1653:   
        for i in range(57):
            for j in range(i+1, 58):
    ...
    problem_fit = mlrose.TSPOpt(length = 58, fitness_fn = fitness_dists, maximize=False)
...
df.to_excel("TSP_brazil_58.xlsx", index=True, startrow=0)