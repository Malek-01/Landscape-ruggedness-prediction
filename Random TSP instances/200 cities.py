...
for k in range(30):
    dist = np.zeros((200,200))
    for i in range(199):
        for j in range(i+1, 200):
           dist[i][j] = random.uniform(1,5)  
    dist_list = [(0,0,0) for x in range(19900)]  
    l = 0
    while l < 19900:   
        for i in range(199):
            for j in range(i+1, 200):
    ...
    problem_fit = mlrose.TSPOpt(length = 200, fitness_fn = fitness_dists, maximize=False)    
...
df.to_excel("TSP_Random_200.xlsx", index=True, startrow=0)
