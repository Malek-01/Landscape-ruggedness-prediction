...
l = list(range(len(y_pred2)))

plt.figure(figsize=(10, 8))
fig, ax1 = plt.subplots()

plt.plot(y_train, label = 'Actual values')
plt.plot(y_pred2, 'o', label= 'GB predictions')

plt.show()
#plt.plot(l, y_test, 'b-', label = 'Real values')
#plt.plot(l, y_pred, 'r-', label = 'RF predictions')
plt.xlabel('Sample'); plt.ylabel('Entropy value'); plt.title('Prediction of landscape ruggedness for TSP using GB (test set)')
plt.legend();
plt.savefig('RF_test_scatter2_train.png')
